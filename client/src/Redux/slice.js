...
import { fetchAddBills, fetchDayBills } from "./actions";
...
const initialState = {
    ...
    totalItems: [],
    storedBillId: '',
    totalDayBills: [], //data stored from an API response
}
...
const slice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchDayBills.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(fetchDayBills.fulfilled, (state, action) => {
                state.status = 'succeeded';
                // Add the fetched bills to the array
                state.totalDayBills = action.payload;
            })
            .addCase(fetchDayBills.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.error.message;
            })
            .addCase(fetchAddBills.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(fetchAddBills.fulfilled, (state, action) => {
                state.status = 'succeeded';
                // Add the fetched bills to the array
                state.totalDayBills = action.payload;
            })
            .addCase(fetchAddBills.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.error.message;
            });
    },
